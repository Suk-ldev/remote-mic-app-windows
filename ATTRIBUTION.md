...
- **macOS 版"语音键模拟 Fn 点按"（HD838A/remote-mic-app，README 公开行为）**：Typeless 这类"点按快捷键开始、再点按结束"的语音工具与遥控器默认的长按行为不兼容，macOS 版为此提供开关，**在语音流开始和排空结束时各发送一次点按**。Windows 版据此实现语音输入快捷键的 `toggle`（单次触发）形态：开始点按一次、**排空结束后**再点按一次（放在排空之后与 macOS 一致，避免虚拟声卡尾音被提前截断）；注入仍走本仓库既有的逐事件 80ms 间隔提交（`HOLD_CHORD_EVENT_GAP`，evidence/p 实证的和弦形态约束），不复用 macOS 代码。配套的"仅微信输入法才做 TSF 会话级激活"边界见上一条。Typeless 与其他第三方工具的真机验证 deferred（需目标软件与 RC001/RC003 主机）。
