...
$out = Join-Path $root 'target\rc003-native-tap'
$reference = Join-Path $out 'reference'
...
$native = Join-Path $root 'native\rc003-tap-probe'
...
if errorlevel 1 exit /b 1
cd /d "$out"
cl /nologo /W4 /WX /O2 /MT /EHsc /std:c++17 /guard:cf /I "$source\include" /LD "$native\tap.cpp" "$source\lib.X64\detours.lib" /link /OUT:sayall-rc003-tap.dll /DYNAMICBASE /NXCOMPAT
if errorlevel 1 exit /b 1
cl /nologo /W4 /WX /O2 /MT /EHsc /std:c++17 /guard:cf "$native\probe.cpp" advapi32.lib cfgmgr32.lib shell32.lib ole32.lib /link /OUT:sayall-rc003-probe.exe /DYNAMICBASE /NXCOMPAT
exit /b %errorlevel%
"@ | Set-Content -LiteralPath $batch -Encoding ascii
$log = Join-Path $out 'build.log'
& $env:ComSpec /d /c $batch *> $log
if ($LASTEXITCODE -ne 0) { Get-Content -LiteralPath $log -Tail 65; throw "Native probe build failed; full log: $log" }
Get-Item (Join-Path $out 'sayall-rc003-tap.dll'), (Join-Path $out 'sayall-rc003-probe.exe') | Select-Object Name,Length
